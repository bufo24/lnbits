...
def _argon2id(password: bytes, salt: bytes) -> bytes:
    # RFC 9106 Argon2id v0x13. Params must match amboss-rails / @ambosstech/payments.
    return Argon2id(
...
def create_master_password_hash(password: str, team_id: str) -> tuple[str, str]:
    """Mirror of amboss-rails `createMasterPasswordHash`.
...
class AmbossWallet(Wallet):
    ...
    async def status(self) -> StatusResponse:
        try:
            ...
            # balance is in the wallet asset's base unit (sats for BTC).
            # ponytail: assumes BTC (sats); msat = sats * 1000.
